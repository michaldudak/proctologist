...
**Posting a review draft** is the one write the app makes to GitHub, and it is the user's, never the agent's (ADR 0003 binds the agent; this happens in the app on a click). The Post button in the panel opens a confirmation that names the pull request and the verdict the review will carry, warns when the pull request's head has moved since the draft was written, and only then runs `gh pr review` under the user's own login with the body as the review text. The draft records when it was posted, so the button gives way to the date and the same draft cannot go up twice; a refusal from GitHub stays in the dialog rather than the console, so a rejected post is never taken for a posted one. Nothing is refreshed afterwards on the app's own initiative: the next refresh brings the review back as GitHub's own fact. Session id kept for future follow-ups.
