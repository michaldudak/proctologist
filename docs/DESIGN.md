...
The analysis is the thorough pass's second product: Markdown in one `analysis` field of the same JSON reply, with `##` sections Background, Intuition, Walkthrough and Points of attention, and Mermaid fences where a picture shows the mechanism better than prose (after Geoffrey Litt's explain-diff brief, minus its quiz). Markdown rather than HTML because the agent writes it reliably inside a JSON reply, the app renders it in its own theme, and nothing the agent wrote ever runs: raw HTML is dropped, diagrams are rendered by Mermaid in strict mode. The analysis lands in the same transaction as its assessment; a thorough reply without one leaves the PR unassessed.
