...
| Review draft       | agent           | Pull requests only. The review as Markdown, in the shape the repository's instructions or skill asked for, beside the agent's own verdict and one-line summary of it; keeps the agent and its session id                       |
...
| Snoozed            | user            | Until the assessment is replaced, until a date, or indefinitely (only clearing it lifts it)                                                                                                                                    |
| Viewed             | user            | Records the last activity the user saw; stops counting once a refresh brings newer activity from another party, while the user's own later activity keeps it                                                                   |
...
**Review draft** (per PR): PR-head worktree, workspace-write sandbox, `review` profile, timeout 30 minutes. The per-repository review instructions text from config is wrapped in the app's frame, which asks for the review itself as Markdown in one `body` field of the JSON reply, shaped by the instructions rather than by the app, beside the agent's verdict and a one-line summary of what it wrote; anything the instructions say about the reply's format is redirected at the body, so a skill that ends with "Markdown, no JSON" still comes back readable. The verdict and the summary are what the panel shows above the rendered body; what the user copies is the body alone, since the verdict is picked in GitHub's review box. Repositories may reference their own skills, which the agent finds in the worktree.

**Posting a review draft** is the one write the app makes to GitHub, and it is the user's, never the agent's (ADR 0003 binds the agent; this happens in the app on a click). The Post button in the panel opens a confirmation that names the pull request and lets the user pick the verdict the review will carry — Approve, Comment or Request changes, preset to the agent's — warns when the pull request's head has moved since the draft was written, and only then runs `gh pr review` under the user's own login with the body as the review text. The draft records when it was posted and with which verdict, so the button gives way to a Posted mark that says when the user overruled the agent, and the same draft cannot go up twice; a refusal from GitHub stays in the dialog rather than the console, so a rejected post is never taken for a posted one. Nothing is refreshed afterwards on the app's own initiative: the next refresh brings the review back as GitHub's own fact. Session id kept for future follow-ups.
...
- Table, shared between both kinds, with its columns, facets, flags and sort supplied per kind rather than branched inside it. Filter bar of search and one menu per facet plus a Show menu, each option carrying the count it would leave, and a columns menu at its end. Number and title can never be hidden. Due is the four reasons the refresh counts — never assessed, failed, changed since, past the `outdated_after_days` cut-off — derived per row from the same config value the refresh uses, so the filter and the Assess button agree on what is outstanding. Snoozed items count for neither: the table hides them and `dueAssessments` leaves them out, so the two read the same number and pressing the button acts on that same set. A full re-assessment passes `includeSnoozed` and is the only thing that reaches past a snooze; a snooze set until the next assessment therefore stands until the user clears it, ticks the item by hand under Also show → Snoozed, or asks for the full run.
  - Pull requests: facets author, next action, priority, area, relevance, status, effort; flags quick wins, due, unassessed, changed, draft, not draft, bot, maintainers, external contributors, yours, review requested, with a note, viewed, not viewed, snoozed, closed; columns number, title with markers, author, next action, priority, area, relevance, status, effort, age, last activity. Default sort Merge, Review, Continue, Close, Nudge author, Decide, Wait; then quick wins, then priority.
  - Issues: facets author, next action, priority, area, relevance, status, effort; flags drop draft and review requested, keep maintainers, external contributors, bot, yours, quick wins, due, unassessed, changed, with a note, viewed, not viewed, snoozed, closed, and gain has assignee, no reply yet, has a linked pull request, first-time reporter; columns number, title with markers, author, next action, priority, area, relevance, status, effort, votes, comments, age, last activity. Votes sort on the net, so a contested issue does not outrank a wanted one on its thumbs up alone, and the thumbs down is shown only when there is one. Default sort Fix, Answer, Reproduce, Request info, Decide, Close, Wait; then quick wins, then priority.
...
- Side panel for the selected item: summary, reasons, evidence, the analysis (when it was written, whether the item has moved on since, its first paragraph, and a Read button), facts, assessment history with verdict changes, note editor, and actions: open on GitHub, mark as viewed (or as not viewed), snooze, re-assess or re-triage (quick), go thorough. Pull requests add drafting a review with an effort picker, viewing or copying the draft, and posting it; issues have no counterpart, and the panel's fact list follows the kind — no diff stats, checks or mergeability on an issue; assignees, milestone, linked pull requests and, once closed, the `state_reason` instead.
