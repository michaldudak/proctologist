...
describe("isViewed", () => {
	...
	it("holds while nothing new has happened", () => {
		expect(isViewed(viewed(), pullRequest())).toBe(true);
	});
	...
	it("stops counting once another party did something newer", () => {
		expect(
			isViewed(
				viewed(),
				pullRequest({ lastActivityAt: "2026-09-05T12:00:00.000Z", lastActivityByUser: false }),
			),
		).toBe(false);
	});
	...
	it("survives the user's own later activity", () => {
		expect(
			isViewed(
				viewed(),
				pullRequest({ lastActivityAt: "2026-09-05T12:00:00.000Z", lastActivityByUser: true }),
			),
		).toBe(true);
	});

	it("is false when there is no mark", () => {
		expect(isViewed(undefined, pullRequest())).toBe(false);
	});
});
